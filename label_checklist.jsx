...
async function uploadPhoto(file) {
  if (!SHEET_URL || SHEET_URL === "__SHEET_URL__") return null;
  try {
    const b64 = await fileToBase64(file);
    const res = await fetch(SHEET_URL, {
      method: "POST",
      body: JSON.stringify({ type: "사진업로드", image: b64 }),
      headers: { "Content-Type": "text/plain" }
    });
...
export default function App(){
  ...
  const submitCL=async()=>{
    ...
    const rec={w:form.name,it:form.item,job:form.job,hc:form.hc||"N/A",
      ...
      photo:photoUrl||""}; // 사진 첨부 여부·URL을 이력에 직접 저장
    ...
    {tab==="cl"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      ...
      <Card style={{padding:"16px 14px"}}>
        ...
        <div style={{marginBottom:10,padding:"12px",background:C.sur,borderRadius:8,border:`1px solid ${C.bor}`}}>
          ...
          {!photoPreview
            ?<label style={{display:"block",textAlign:"center",padding:"14px",border:`1px dashed ${C.bor}`,
              borderRadius:8,color:C.blu,fontSize:12,fontWeight:700,cursor:"pointer"}}>
              사진 촬영 / 선택
