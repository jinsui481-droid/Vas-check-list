...
const EDU=[
  ...
  {icon:"🔖",title:"착인(스탬프) 작업 주의사항",
   items:["착인 전 제조번호·유효기간이 작업지시와 일치하는지 확인","날짜·번호 형식(자릿수·연월일 순서)이 기준과 맞는지 점검","스탬프가 용기 표시사항(성분·용량 등)을 가리지 않게 위치 선정","잉크 번짐·흐림·이중 착인 없이 선명하게 1회 날인","첫 5개 착인 후 번호·날짜·위치·선명도 자체 검사 후 진행","잉크 부족·날짜 설정 변경 시 반드시 첫 결과물 재확인"],
   tip:"착인은 라벨 오부착과 같은 수준의 클레임 리스크다. 잘못된 번호·날짜는 회수 사유가 되고, 표시사항을 가리면 화장품법 위반이다. 날짜 변경 직후가 가장 위험하니 첫 결과물을 반드시 확인한다."},
  {icon:"📌",title:"품질 기준 요점",
...
const RNR=[
  {role:"직원(관리자)",icon:"🧑‍💼",color:"acc",
   sub:"최종 판단권자 · 출고 가부 결정 · 앱 관리자",
   resp:["라인 전체 운영 총괄 및 최종 의사결정","출고 가부 최종 판단 (품질 이상 시 출고 보류 결정)","치명적 실수·재작업 발생 시 최종 대응 방향 결정","앱 관리자 기능으로 운영 현황·불량 패턴·건의 관리"],
   key:["검수·반장에서 올라온 이상 건 최종 판정","불량 패턴 데이터 기반 개선 방향 수립","인력·물량·프로모션 대응 계획 총괄"],
   hand:"라인의 최종 책임자. 현장 역할들이 걸러낸 정보를 종합해 출고 여부와 개선 방향을 결정한다."},
  {role:"반장",icon:"🎖️",color:"red",
   sub:"라벨 불출 · 1차 제품 검수 · 라벨-제품 매칭",
   resp:["작업가이드 기준 라벨 불출","1차 제품 검수 (품목·상태 확인)","라벨과 제품의 품목·로트 매칭 확인","이상 발견 시 직원(관리자)에게 보고"],
   key:["라벨 불출 시 품명·로트를 제품과 대조 (교차부착 1차 차단)","제품 상태(오품·파손·오염) 1차 선별","매칭 단계에서 라벨-제품 불일치 차단"],
   hand:"라벨과 제품이 처음 만나는 지점. 여기서 매칭이 틀리면 라인 전체가 오염되므로 1차 검수와 매칭이 핵심이다."},
  {role:"조장",icon:"🧭",color:"blu",
   sub:"2차 라벨-제품 매칭 · 라인 작업 지시",
   resp:["2차 라벨-제품 매칭 확인 (반장 다음 단계 재확인)","라인 작업 지시 및 작업자 배치","작업 진행·속도 관리","작업 중 문제 1차 대응, 필요 시 관리자 보고"],
   key:["반장이 넘긴 라벨-제품을 한 번 더 대조","작업자(일용직 포함)에게 작업 방법 지시","작업 전환 시 첫 제품 상태 확인"],
   hand:"매칭의 2차 확인자이자 라인 지시자. 반장-조장 이중 매칭으로 교차부착을 두 번 거른다."},
  ...
  {role:"검수자",icon:"🔍",color:"grn",
   ...
   hand:"작업이 도는 내내 이상을 잡는 상시 품질 게이트. 일용직 비중이 높을수록 이 지속 확인이 품질을 지탱한다."},
      ...
      <Card style={{marginBottom:12,borderColor:C.acc,borderWidth:1}}>
        <CardH style={{justifyContent:"space-between",alignItems:"center"}}>
          ...
          <span onClick={()=>{
            if(rnrEdit){setRnrEdit(false);}
              ...
              const inp=prompt("관리자 PIN을 입력하세요");
              if(inp===pin){setRnrEdit(true);setRnrShow(true);}
              else if(inp!=null)showToast("PIN이 올바르지 않습니다",C.red);
            }
        ...
        {rnrShow&&<div style={{padding:"0 12px 12px"}}>
          <div style={{fontSize:9.5,color:C.mut,marginBottom:10,lineHeight:1.5}}>
            {rnrEdit?"편집 모드: 각 역할의 부제·책임·핵심행동을 수정할 수 있습니다. 한 줄씩 고치고 완료를 누르세요.":"각 역할이 무엇을 책임지고 어디서 품질을 거르는지 정리했습니다. 일용직이 바뀌어도 이 골격이 라인을 지탱합니다. 역할을 눌러 상세를 보세요."}
          </div>
          {rnr.map((r,i)=>{
          ...
          <div style={{marginTop:10,padding:"10px",background:C.sur,borderRadius:8,border:`1px solid ${C.bor}`}}>
            ...
            <div style={{fontSize:9.5,color:C.txt,lineHeight:1.7}}>
              <b style={{color:C.red}}>반장</b>(라벨 불출·1차 검수·매칭) → <b style={{color:C.blu}}>조장</b>(2차 매칭·작업 지시) → 부착작업자(1차 자기확인) → 부조장(작업 중 보조 확인) → <b style={{color:C.grn}}>검수자(작업 중 상시 확인)</b> → <b style={{color:C.acc}}>직원/관리자(출고 가부 최종 판정)</b>
            </div>
            <div style={{fontSize:8.5,color:C.mut,marginTop:5,lineHeight:1.4}}>
              ※ 라벨-제품 매칭을 반장·조장이 두 번 거르고, 작업 중에는 부조장·검수자가 상시 확인한다. 출고 가부의 최종 판단은 직원(관리자)이 한다.
            </div>
          </div>
        </div>}
      </Card>
