...
export default function App(){
  ...
  const initSecs=()=>{
    ...
    return {s1:mk(CL0.s1),s2:mk(CL0.s2),s3:mk(CL0.s3),s4:mk(CL0.s4)};
  };
  ...
  return <div style={{background:C.bg,minHeight:"100vh",color:C.txt,fontFamily:"sans-serif",paddingBottom:80}}>
      ...
      return <div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
        {jobs.length===0
          ?<Card style={{padding:"24px 14px",textAlign:"center"}}>
            ...
            <div style={{fontSize:11,color:C.mut,lineHeight:1.6}}>관리자 탭에서 작업을 선택해<br/>인건비·작업단가를 입력하면<br/>여기에 목표·현재 페이스가 표시됩니다.</div>
          </Card>
          :jobs.map(x=>{
    ...
    {tab==="edu"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      {EDU.map((e,i)=>(
        <Card key={i}>
          <CardH><span style={{fontSize:13,fontWeight:700,color:C.txt}}>{e.title}</span></CardH>
    ...
    <PinModal open={editPinOpen} onClose={()=>setEditPinOpen(false)}
      title="항목 편집 PIN"
      onOk={v=>{
        if(v===PIN){setEditMode(true);setEditPinOpen(false);}
    ...
    <PinModal open={mgrPinOpen} onClose={()=>setMgrPinOpen(false)}
      title="관리자 PIN"
      onOk={v=>{
        if(v===PIN){setMgrIn(true);setMgrPinOpen(false);}
        else showToast("PIN이 올바르지 않습니다",C.red);
      }}/>
