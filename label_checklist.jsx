...
export default function App(){
  ...
  const submitCL=async()=>{
    ...
    if(photo){
      ...
      setUploading(false);
      if(!photoUrl){showToast("사진 업로드 실패. 사진 없이 제출하거나 다시 시도하세요.",C.red);return;}
    }
    const ts=Date.now();
    const rec={w:form.name,it:form.item,job:form.job,hc:form.hc||"N/A",
      ws:form.ws,we:"",dt:form.date,d:done,a:total,f:done===total,ts,
      photo:photoUrl||"",
      labelNA:notLabel}; // 사진 URL + 라벨 작업 아님 여부(라벨 점검 N/A)
    ...
    if(photoUrl){
      fbPush("photos",{w:form.name,it:form.item,job:form.job||"미지정",url:photoUrl,dt:form.date,ts});
    }
    ...
    setNotLabel(false);
    setSecs(initSecs());
    showToast(photoUrl?"✅ 제출 완료! (사진 포함)":"✅ 제출 완료! 안전한 작업 시작하세요.");
  };
