...
const EDU=[
  {icon:"🔴",title:"치명적 실수 — 즉시 중단·보고",
   items:["라벨 교차부착 (A품목 라벨이 B품목에 부착)","로트번호·유효일자 오부착 또는 가림","미부착 상태로 완성품 처리","세트 구성 품목·로트 불일치"],
   tip:"위 네 가지는 모두 고객 클레임으로 직결되는 치명적 오류다. 발견 즉시 작업을 멈추고 관리자에게 보고한다. 혼자 판단해 넘기지 않는다."},
  {icon:"🟡",title:"즉시 보고 — 작업은 계속, 알림 필수",
   items:["라벨 일부 들뜸·기포·접힘 다수 발생","피킹지 수량과 실물 수량 불일치","작업가이드·기준 샘플 누락 또는 불명확","용기 파손·오염 등 제품 이상 발견"],
...
export default function App(){
  ...
  useEffect(()=>{
    fbOn("checklist",(data)=>{
      ...
      const local=gs("lh")||[];
      const merged=[...data];
      // 로컬에만 있고 Firebase에 아직 없는 항목(ts 기준) 보존
    ...
    {tab==="mgr"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      {!mgrIn
        ...
        :<>
          ...
          <Card style={{padding:"14px"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:8}}>📷 제출된 라벨 샘플 사진</div>
            {photos.length===0
