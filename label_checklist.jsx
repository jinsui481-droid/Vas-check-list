...
const SHEET_URL = "https://script.google.com/macros/s/AKfycbwCtp1ihaSF3Z8_IqZ_GVKhuPFk4QhBn8I_m5yIOFkTX4vYDfD_HRYRZH7O576qWok-/exec";
const IMGBB_KEY = "cd2edabe5e2d26dc259dcfbc59a10204"; // 무료 이미지 호스팅 (노출 시 재발급 가능)
const RECENT_DAYS = 60; // 화면에 로딩할 최근 기간(일). 과거 기록은 관리자 '과거 조회'로 검색
...
async function fbGetOnce(path) {
  // 일회성 전체 읽기 (과거 기록 조회용)
  try {
    const db = await getDB();
    const { ref, get } = await import("https://www.gstatic.com/firebasejs/12.15.0/firebase-database.js");
    const snap = await get(ref(db, path));
    const val = snap.val();
    return val ? Object.values(val) : [];
  } catch(e) { console.error("fbGetOnce error:", e); return []; }
}
async function fbUpdateByTs(path, ts, patch) {
  // ts 필드로 레코드를 찾아 일부 필드만 업데이트 (제출 후 사진 추가용)
  try {
    const db = await getDB();
    ...
    // ts가 일치하는 키 찾기
    let foundKey = null;
    Object.keys(val).forEach(k => { if (val[k] && val[k].ts === ts) foundKey = k; });
    if (!foundKey) return false;
    await update(ref(db, path + "/" + foundKey), patch);
    return true;
  } catch(e) { console.error("fbUpdateByTs error:", e); return false; }
}
async function fbDeleteByTs(path, ts) {
  // ts 필드로 레코드를 찾아 삭제
  try {
    const db = await getDB();
    const { ref, get, remove } = await import("https://www.gstatic.com/firebasejs/12.15.0/firebase-database.js");
    const snap = await get(ref(db, path));
    const val = snap.val();
    if (!val) return false;
    let foundKey = null;
    Object.keys(val).forEach(k => { if (val[k] && val[k].ts === ts) foundKey = k; });
    if (!foundKey) return false;
    await remove(ref(db, path + "/" + foundKey));
    return true;
  } catch(e) { console.error("fbDeleteByTs error:", e); return false; }
}
...
/* ── 사진 업로드 (Apps Script 중계 → imgBB) ── */
function fileToBase64(file) {
  return new Promise((resolve, reject) => {
    const r = new FileReader();
    r.onload = () => resolve(r.result.split(",")[1]); // 헤더 제거
    r.onerror = reject;
...
async function uploadPhoto(file) {
  try {
    ...
    // imgBB는 CORS를 허용하므로 브라우저에서 직접 업로드 가능
...
s1:[
  ...
  {t:"작업대 위 라벨은 1개 품목만 비치",d:"외관이 유사한 제품 라벨은 혼입 위험이 높다. 반드시 1종만 올려놓는다.",r:true},
...
s4:[
  ...
  {t:"중량 검수 — 완성품 중량이 기준 범위 내인지 확인",d:"내용물 누락·과충전·구성품 누락은 중량으로 잡힌다. 기준 중량과 허용 오차를 벗어나면 작업 중단 후 보고한다.",r:true},
  {t:"작업대 청결 및 이전 라벨 잔여물 완전 제거",d:"이전 품목 라벨 잔여물, 먼지가 없는지 확인한다. 유사 외관 제품 혼입 특히 주의.",r:false},
  {t:"재작업 발생 시 유형 구분하여 즉시 관리자 보고",d:"교차부착·미부착·오부착 중 유형을 구분하고 수량을 파악하여 즉시 보고한다.",r:false}
]};

const SECS=[
  {id:"s1",label:"치명적",lc:C.red,bc:"#3d1a1a",title:"① 라벨 교차부착 방지"},
  {id:"s2",label:"치명적",lc:C.red,bc:"#3d1a1a",title:"② 라벨 미부착 방지"},
  {id:"s3",label:"치명적",lc:C.red,bc:"#3d1a1a",title:"③ 오부착 방지"},
  {id:"s4",label:"필수점검",lc:C.orn,bc:"#3a2d0f",title:"④ 공통 점검"}
];
...
const EDU=[
  {icon:"🔴",title:"치명적 실수 — 즉시 중단·보고",
   items:["라벨 교차부착 (A품목 라벨이 B품목에 부착)","로트번호·유효일자 오부착 또는 가림","미부착 상태로 완성품 처리","세트 구성 품목·로트 불일치","들뜸·기포·접힘이 다수 반복 발생 (라벨·접착·작업방식 이상 신호)"],
   tip:"위 항목은 모두 고객 클레임 또는 공정 이상으로 직결된다. 발견 즉시 작업을 멈추고 관리자에게 보고한다. 특히 들뜸·기포가 다수 반복되면 개별 재압착으로 넘기지 말고 원인부터 확인한다."},
  {icon:"🟡",title:"즉시 보고 — 작업은 계속, 알림 필수",
   items:["피킹지 수량과 실물 수량 불일치","작업가이드·기준 샘플 누락 또는 불명확","용기 파손·오염 등 제품 이상 발견","동일 유형 경미한 오류가 반복되는 조짐"],
   tip:"작업을 중단할 정도는 아니지만 반드시 관리자에게 알려야 하는 사안. 보고 없이 임의 진행하면 나중에 더 큰 문제가 된다."},
  {icon:"🟢",title:"주의하여 작업 — 자체 수정 가능",
   items:["라벨이 살짝 비뚤어짐 (로트·정보 노출엔 지장 없음)","들뜸·기포가 경미해 재압착으로 해결 가능","작업대 위 먼지·잔여물 (즉시 정리)","라벨 방향 헷갈림 (부착 전 재확인)"],
   tip:"작업자가 그 자리에서 바로 고칠 수 있는 수준. 다만 같은 실수가 반복되면 패턴을 관리자와 공유한다."},
  ...
  {icon:"🦺",title:"안전·취급 주의사항",
   items:["용기를 잡는 손으로 안정적으로 고정 후 부착(미끄럼·낙하 주의)","유리·내압 용기 파손 시 맨손 접촉 금지, 관리자 통보","내용물 누출·오염 발견 시 작업 중단·격리 후 보고","반복 자세 누적 시 정해진 휴식 시간 준수","작업대 물기·이물질 즉시 제거(미끄럼·혼입 방지)"],
   tip:"이상 발견 시 작업을 멈추고 보고하는 것이 원칙."},
  {icon:"🆘",title:"돌발상황 대응",
   items:["라벨 부족 시 임의 보충 금지, 관리자에게 요청","인쇄 흐림·손상 라벨은 사용 금지, 별도 분리","피킹지와 실물(품목·수량) 불일치 시 작업 중단·확인","부착 라벨이 불명확하면 진행 전 확인","장비 이상 시 무리한 조작 금지, 보고"],
   tip:"불확실한 상황은 진행보다 확인이 먼저. 임의 판단으로 진행하지 않는다."},
  {icon:"🔖",title:"착인(스탬프) 작업 주의사항",
   items:["착인 전 제조번호·유효일자가 작업지시와 일치하는지 확인","날짜·번호 형식(자릿수·연월일 순서)이 기준과 맞는지 점검","정확한 위치에 착인 (지정 자리를 벗어나지 않게)","용량·분리배출 등 다른 표시정보를 가리지 않게 위치 선정","잉크 번짐·흐림·이중 착인 없이 선명하게 1회 날인","수시 샘플링 — 번호·날짜·위치·선명도를 작업 중 계속 자체 검사","잉크 부족·날짜 설정 변경 직후 첫 결과물 반드시 재확인"],
   tip:"착인은 라벨 오부착과 같은 수준의 클레임 리스크다. 잘못된 번호·날짜는 회수 사유가 되고, 용량·분리배출 등 표시사항을 가리면 화장품법 위반이다. 날짜 변경 직후가 가장 위험하니 첫 결과물을 반드시 확인한다."},
  {icon:"📌",title:"품질 기준 요점",
   items:["라벨 1건 오류 = 해당 제품은 불량 처리","로트·유효일자는 화장품법상 필수 표기(가림 금지)","경미해 보이는 오류도 누적 시 클레임·반품으로 연결","샘플 검사로는 전수를 못 잡으므로 작업 단계에서 정확히","기준이 모호하면 관리자에게 확인 후 진행"],
   tip:"체크리스트·기준 준수가 재작업을 줄여 결과적으로 처리량 손실을 막는다."}
];

/* ── 라인 역할별 R&R (책임·핵심행동·인계포인트) ── */
const RNR=[
  {role:"반장",icon:"🎖️",color:"red",
   sub:"인원 배치 · 라벨 불출 · 1차 제품 검수 · 라벨-제품 매칭",
   resp:["라인 인원 배치 (작업별 인력 배정)","작업가이드 기준 라벨 불출","1차 제품 검수 (품목·상태 확인)","라벨과 제품의 품목·로트 매칭 확인","이상 발견 시 보고 및 대응"],
   key:["인원 배치 시 작업 난이도·숙련도 고려","라벨 불출 시 품명·로트를 제품과 대조 (교차부착 1차 차단)","제품 상태(오품·파손·오염) 1차 선별","매칭 단계에서 라벨-제품 불일치 차단"],
   hand:"인원 배치와 라벨·제품 공급의 출발점. 인력을 배정하고, 라벨-제품 매칭이 틀리면 라인 전체가 오염되므로 1차 검수와 매칭이 핵심이다."},
  {role:"조장",icon:"🧭",color:"blu",
   sub:"배치 인원 작업 지시 · 2차 라벨-제품 매칭",
   resp:["반장이 배치한 인원에게 작업 지시 및 작업 방법 지도","2차 라벨-제품 매칭 확인 (반장 다음 단계 재확인)","작업 진행·속도 관리","작업 중 문제 1차 대응 및 보고"],
   key:["배치된 작업자에게 작업 방법·기준 지시","반장이 넘긴 라벨-제품을 한 번 더 대조","작업 전환 시 첫 제품 상태 확인"],
   hand:"반장이 배치한 인원을 실제로 작업시키는 라인 지시자이자 매칭 2차 확인자. 반장-조장 이중 매칭으로 교차부착을 두 번 거른다."},
  {role:"부조장",icon:"🤝",color:"blu",
   sub:"조장 보조 · 작업 중 이상 확인",
   resp:["작업 중 조장 보조","작업 진행 중 이상 여부 수시 확인","작업자 결원·물량 변동 시 유연 투입","자재·라벨 보충 및 작업대 정리 지원"],
   key:["작업 중 라벨·제품 이상 발견 시 조장에 즉시 전달","라인 병목 구간 우선 지원","작업대 1품목 1트레이 원칙 유지"],
   hand:"조장을 보조하며 작업 중 이상을 잡아내는 역할. 라인이 도는 동안 실시간으로 살핀다."},
  {role:"피킹",icon:"📥",color:"orn",
   sub:"제품 공급 · 수량·품목 정확성 책임",
   resp:["피킹지 기준 제품(용기) 품목·수량 정확히 불출","공급 제품과 피킹지 품목·수량 일치 확인","수량 과부족 없이 정확한 물량 라인 공급","오품·파손·오염 제품 선별 제거"],
   key:["제품 불출 시 피킹지와 품목·수량 1차 대조","수량 부족·과다 발견 시 즉시 조장 보고","서로 다른 품목 제품 혼입 방지"],
   hand:"제품 수량·품목 정확성의 출발점. 여기서 수량이 틀리면 미부착·수량오류로 이어진다. (라벨은 반장이 별도 공급)"},
  {role:"검수자",icon:"🔍",color:"grn",
   sub:"작업 중 이상 여부 지속 확인",
   resp:["작업이 진행되는 동안 이상 여부를 계속 확인","부착·착인·중량 등 완성 상태 점검","이상 발견 시 작업 중단 요청 및 재작업 분류","검수 결과를 조장·관리자에 보고"],
   key:["로트·유효일자 가림 여부 집중 확인 (최빈 불량)","들뜸·기포·접힘 다수 발생 시 개별처리 아닌 원인보고","작업 중 상시 확인 (사후 일괄검사 아님)"],
   hand:"작업이 도는 내내 이상을 잡는 상시 품질 게이트. 인원이 자주 바뀌는 환경일수록 이 지속 확인이 품질을 지탱한다."},
  {role:"부착 작업자",icon:"✋",color:"mut",
   sub:"라벨 부착",
   resp:["배정된 품목의 작업가이드·기준 샘플대로 부착","작업대에 배정 품목 라벨만 비치","부착 후 본인 1차 확인 (위치·방향·들뜸)","이상·의문 발생 시 임의 진행 말고 조장에 문의"],
   key:["다른 품목 라벨이 섞이면 즉시 중단·보고","로트·유효일자 가리지 않게 부착","부족·불량 라벨은 임의 사용 금지"],
   hand:"실제 부착 실행. 매일 인원이 바뀔 수 있으므로 '기준대로, 모르면 묻기'가 핵심. 판단은 조장·검수가 보완한다."}
]
...
function Badge({label,lc,bc}){
  return <span style={{fontSize:9,fontWeight:900,padding:"3px 7px",borderRadius:99,
    background:bc,color:lc,border:`1px solid ${lc}`,flexShrink:0,whiteSpace:"nowrap"}}>{label}</span>;
}
...
export default function App(){
  ...
  const initSecs=()=>{
    ...
    // 저장된 데이터가 유효한지 검사 (항목 텍스트가 실제로 있는지)
    const validSaved=saved&&["s1","s2","s3","s4"].every(k=>Array.isArray(saved[k]))
      &&["s1","s2","s3","s4"].some(k=>saved[k].some(it=>it&&it.t&&it.t.trim()));
    const base=validSaved?saved:CL0;
    const mk=arr=>(arr||[]).filter(i=>i&&i.t&&i.t.trim()).map(i=>({...i,checked:false,id:Math.random()}));
  ...
  const [notLabel,setNotLabel]=useState(false); // 라벨 작업 아님 (라벨 섹션 N/A 처리)
  ...
  const [rnrOpen,setRnrOpen]=useState(null); // 펼친 역할 인덱스
  const [rnrShow,setRnrShow]=useState(true); // R&R 섹션 표시
  const [rnr,setRnr]=useState(()=>{
    const saved=gs("rnrData");
    // 구버전(직원 카드 등) 저장돼 있으면 무시하고 최신 기본값 사용
    if(Array.isArray(saved)&&!saved.some(r=>r&&(r.role==="직원(관리자)"||r.role==="검수"||(r.sub&&r.sub.includes("최종 판단권자"))))){
      return saved;
    }
    return RNR;
  }); // R&R 데이터(편집 가능)
  const [rnrEdit,setRnrEdit]=useState(false); // R&R 편집 모드
  const eduEditRef=useRef(false);
  useEffect(()=>{eduEditRef.current=eduEdit;},[eduEdit]);
  const rnrEditRef=useRef(false);
  useEffect(()=>{rnrEditRef.current=rnrEdit;},[rnrEdit]);
  const [eduPinOpen,setEduPinOpen]=useState(false);
  const [rnrPinOpen,setRnrPinOpen]=useState(false);
  ...
  const saveRnr=n=>{setRnr(n);ss("rnrData",n);fbSet("rnrData/current",n);};

  /* 작업명별 가이드 링크: { "작업명": "url" } */
  const [guides,setGuides]=useState(()=>gs("guides")||{});
  const [photos,setPhotos]=useState([]); // 제출된 라벨 사진 목록
  const [photoFilter,setPhotoFilter]=useState(""); // 작업명 필터
  const [photoView,setPhotoView]=useState(""); // 확대 보기 URL
  const [noPhotoOnly,setNoPhotoOnly]=useState(false); // 사진 미첨부만 필터
  // 과거 기록 조회
  const [archOpen,setArchOpen]=useState(false); // 과거 조회 모달
  const [archFrom,setArchFrom]=useState(""); // 시작일 (YYYY-MM-DD)
  const [archTo,setArchTo]=useState(""); // 종료일
  const [archKw,setArchKw]=useState(""); // 작업자/품목/작업명 키워드
  const [archResult,setArchResult]=useState(null); // 조회 결과 배열
  const [archLoading,setArchLoading]=useState(false);
  const guidesRef=useRef({});
  useEffect(()=>{guidesRef.current=guides;},[guides]);
  const saveGuides=g=>{setGuides(g);ss("guides",g);fbSet("guides/current",g);};
  ...
  /* 서명 폼 */
  const [form,setForm]=useState({name:gs("lastName")||"",item:"",job:"",hc:"",ws:"",date:now});
  const [photo,setPhoto]=useState(null); // 첨부 사진 파일
  const [photoPreview,setPhotoPreview]=useState(""); // 미리보기 URL
  const [uploading,setUploading]=useState(false);
  const photoInputRef=useRef(null);
  const addPhotoRef=useRef(null); // 제출 후 사진 추가용 input
  const [addPhotoTs,setAddPhotoTs]=useState(null); // 사진 추가 대상 제출 ts
  ...
  const [sugg,setSugg]=useState(()=>gs("sugg")||[]); // 건의사항
  const [suggF,setSuggF]=useState({w:"",text:""}); // 건의 작성 폼

  // Firebase 실시간 구독 (마운트 시 1회)
  useEffect(()=>{
    fbOn("checklist",(data)=>{
      // Firebase가 빈 배열을 줘도 로컬 데이터를 날리지 않도록 병합
      const local=gs("lh")||[];
      const merged=[...data];
      local.forEach(l=>{ if(l&&l.ts&&!merged.some(d=>d.ts===l.ts)) merged.push(l); });
      // 부하 방지: 최근 RECENT_DAYS일치만 화면에 로딩 (과거는 '과거 조회'로 검색)
      const cutoff=Date.now()-RECENT_DAYS*24*60*60*1000;
      const recent=merged.filter(x=>x.ts&&x.ts>=cutoff).sort((a,b)=>b.ts-a.ts);
      setHist(recent);
      ss("lh",recent);
    });
    fbOn("rework",(data)=>{
    ...
    fbOnRaw("clData/current",(data)=>{
      // 유효성 검사: 4개 섹션이 모두 배열이고 항목 텍스트가 실제로 있어야 적용
      const valid=data&&["s1","s2","s3","s4"].every(k=>Array.isArray(data[k]))
        &&["s1","s2","s3","s4"].some(k=>data[k].some(it=>it&&it.t&&it.t.trim()));
      if(valid&&!editModeRef.current){
        ss("clData",data);
        const mk=arr=>(arr||[]).filter(i=>i&&i.t&&i.t.trim()).map(i=>({...i,checked:false,id:Math.random()}));
        setSecs({s1:mk(data.s1),s2:mk(data.s2),s3:mk(data.s3),s4:mk(data.s4)});
      }else if(data!==null&&!valid){
        // Firebase에 비정상(빈) 데이터가 있으면 원본 CL0로 자동 복구
        fbSet("clData/current",CL0);
      }
    });
    // 교육 내용 실시간 구독
    ...
    fbOnRaw("rnrData/current",(data)=>{
      if(data&&!rnrEditRef.current){
        const arr=Array.isArray(data)?data:Object.values(data);
        // 구버전 감지: '직원(관리자)' 같은 옛 역할이 섞여 있으면 무시하고 최신 기본값으로 자동 복구
        const hasLegacy=arr.some(r=>r&&(r.role==="직원(관리자)"||r.role==="검수"||(r.sub&&r.sub.includes("최종 판단권자"))));
        if(hasLegacy){
          fbSet("rnrData/current",RNR); // Firebase의 옛 데이터를 최신으로 덮어씀
          setRnr(RNR);ss("rnrData",RNR);
          return;
        }
        ss("rnrData",data);
        setRnr(arr);
    ...
    // 제출된 라벨 사진 실시간 구독
    fbOn("photos",(data)=>{
      setPhotos([...data].sort((a,b)=>b.ts-a.ts));
    });
  },[]);
  ...
  /* 진행률 - 라벨 작업 아님이면 라벨 섹션(s1·s2·s3) 제외, 공통점검(s4)만 카운트 */
  const labelSecs=["s1","s2","s3"];
  const activeItems=notLabel
    ? (secs.s4||[]) // 라벨 작업 아니면 공통점검만
    : Object.values(secs).flat();
  const done=activeItems.filter(i=>i.checked).length;
  const total=activeItems.length;

  /* 체크리스트 저장 */
  const saveSecs=n=>{
    const clean=arr=>(arr||[]).filter(i=>i&&i.t&&i.t.trim()).map(({t,d,r})=>({t,d:d||"",r:!!r}));
    const data={s1:clean(n.s1),s2:clean(n.s2),s3:clean(n.s3),s4:clean(n.s4)};
    // 전 섹션이 비면 저장하지 않음 (실수로 전체 삭제 방지)
    const hasAny=["s1","s2","s3","s4"].some(k=>data[k].length>0);
    if(!hasAny)return;
    ss("clData",data);
  ...
  const submitCL=async()=>{
    ...
    if(uploading)return;
    let photoUrl="";
    // 사진이 첨부됐으면 먼저 업로드 (실패해도 제출은 계속 진행)
    if(photo){
      setUploading(true);
      showToast("사진 업로드 중...",C.blu);
      try { photoUrl=await uploadPhoto(photo)||""; } catch(_) { photoUrl=""; }
      setUploading(false);
    }
    const ts=Date.now();
    // 시트 전송용 레코드 (사진 필드 제외 - 시트 컬럼과 일치)
    const sheetRec={w:form.name,it:form.item,job:form.job,hc:form.hc||"N/A",
      ws:form.ws,dt:form.date,d:done,a:total,f:done===total};
    // 앱/Firebase 저장용 레코드 (사진·라벨NA 포함)
    const rec={...sheetRec,we:"",ts,photo:photoUrl||"",labelNA:notLabel};
    // localStorage (오프라인 대비) - 항상 먼저 저장해 화면에 즉시 반영
    const h=[rec,...hist];setHist(h);ss("lh",h);
    // Firebase 저장
    try { fbPush("checklist",rec); } catch(_){}
    // 구글 시트 전송 (사진 필드 없는 sheetRec로 - 시트 충돌 방지)
    try { toSheet("체크리스트제출",sheetRec); } catch(_){}
    // 사진이 있으면 photos에도 별도 저장 (작업별 갤러리용)
    if(photoUrl){
      try { fbPush("photos",{w:form.name,it:form.item,job:form.job||"미지정",url:photoUrl,dt:form.date,ts}); } catch(_){}
    }
    ss("lastName",form.name); // 이름 자동 저장
    setForm({name:form.name,item:"",job:"",hc:"",ws:"",date:now});
    setPhoto(null);setPhotoPreview("");
    setNotLabel(false);
    setSecs(initSecs());
    if(photo&&!photoUrl) showToast("제출 완료! (사진 업로드는 실패해 미첨부 처리됨)",C.orn);
    else showToast(photoUrl?"✅ 제출 완료! (사진 포함)":"✅ 제출 완료! 안전한 작업 시작하세요.");
  };

  /* 마감 */
  // 과거 기록 조회 (Firebase 전체에서 날짜·키워드 필터)
  const runArchSearch=async()=>{
    setArchLoading(true);
    setArchResult(null);
    const all=await fbGetOnce("checklist");
    let res=all.filter(x=>x&&x.ts);
    // 날짜 범위 필터 (archFrom/archTo는 YYYY-MM-DD)
    if(archFrom){
      const from=new Date(archFrom+"T00:00:00").getTime();
      res=res.filter(x=>x.ts>=from);
    }
    if(archTo){
      const to=new Date(archTo+"T23:59:59").getTime();
      res=res.filter(x=>x.ts<=to);
    }
    // 키워드 필터 (작업자/품목/작업명)
    if(archKw.trim()){
      const kw=archKw.trim().toLowerCase();
      res=res.filter(x=>
        (x.w||"").toLowerCase().includes(kw)||
        (x.it||"").toLowerCase().includes(kw)||
        (x.job||"").toLowerCase().includes(kw));
    }
  ...
  // 제출 후 사진 추가 (이미 제출된 건에 사진만 붙이기)
  const addPhotoToRec=async(file,targetTs)=>{
    if(!file||!targetTs)return;
    setAddingPhoto(true);
    showToast("사진 업로드 중...",C.blu);
    let url="";
    try { url=await uploadPhoto(file)||""; } catch(_) { url=""; }
    setAddingPhoto(false);
    if(!url){showToast("사진 업로드 실패. 다시 시도하세요.",C.red);return;}
    // 대상 레코드 찾기
    const target=hist.find(x=>x.ts===targetTs);
    if(!target){showToast("대상 작업을 찾을 수 없습니다",C.red);return;}
    // 로컬 hist 갱신
    const updated=hist.map(x=>x.ts===targetTs?{...x,photo:url}:x);
    setHist(updated);ss("lh",updated);
    // Firebase checklist의 해당 레코드 갱신 (ts로 키 찾아 업데이트)
    try { fbUpdateByTs("checklist",targetTs,{photo:url}); } catch(_){}
    // photos 갤러리에도 추가
    try { fbPush("photos",{w:target.w,it:target.it,job:target.job||"미지정",url,dt:target.dt,ts:Date.now()}); } catch(_){}
    setAddPhotoTs(null);
  ...
  /* 건의사항 제출 (작업자, 실명) */
  const submitSugg=()=>{
    if(!suggF.w.trim()){showToast("이름을 입력하세요",C.red);return;}
    if(!suggF.text.trim()){showToast("건의 내용을 입력하세요",C.red);return;}
    const rec={w:suggF.w.trim(),text:suggF.text.trim(),dt:now,read:false,done:false,ts:Date.now()};
    const arr=[rec,...sugg];setSugg(arr);ss("sugg",arr);
    fbPush("suggestion",rec);
    setSuggF({w:suggF.w,text:""}); // 이름은 유지
    ss("lastName",suggF.w.trim());
    showToast("💬 건의가 전달되었습니다");
  };

  /* 건의 상태 변경 (관리자) - read/done */
  const suggSetState=(ts,patch)=>{
    const arr=sugg.map(x=>x.ts===ts?{...x,...patch}:x);
    setSugg(arr);ss("sugg",arr);
    try{ fbUpdateByTs("suggestion",ts,patch); }catch(_){}
  };

  /* 건의 삭제 (관리자) */
  const suggDelete=(ts)=>{
    const arr=sugg.filter(x=>x.ts!==ts);
    setSugg(arr);ss("sugg",arr);
    try{ fbDeleteByTs("suggestion",ts); }catch(_){}
  };
  ...
  const TABS=[["cl","✅ 체크"],["goal","🎯 목표"],["edu","📚 교육"],["rw","⚠️ 재작업"],["sugg","💬 건의"],["mgr","🔑 관리자"]];
  ...
  return <div style={{background:C.bg,minHeight:"100vh",color:C.txt,fontFamily:"sans-serif",paddingBottom:80}}>
    ...
    {tab==="cl"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      ...
      <div onClick={()=>{
        const next=!notLabel;
        setNotLabel(next);
        if(next)showToast("라벨 섹션은 '해당없음'으로 기록됩니다. 봉합라벨은 라벨 작업에 포함되니 체크를 해제하세요.",C.orn);
      }} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",marginBottom:10,
        background:notLabel?"rgba(243,156,18,.12)":C.card,borderRadius:10,
        border:`1px solid ${notLabel?C.orn:C.bor}`,cursor:"pointer"}}>
        <div style={{width:22,height:22,flexShrink:0,borderRadius:6,
          border:`2px solid ${notLabel?C.orn:C.bor}`,background:notLabel?C.orn:"transparent",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#000"}}>
          {notLabel?"✓":""}
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:12,fontWeight:700,color:notLabel?C.orn:C.txt}}>이 작업은 라벨 작업이 아님</div>
          <div style={{fontSize:10,color:C.mut,marginTop:2,lineHeight:1.4}}>체크 시 라벨 점검(①②③)은 해당없음 처리되고, 공통 점검(④)만 진행합니다. <b style={{color:C.orn}}>봉합라벨은 라벨 작업입니다.</b></div>
        </div>
      </div>
      ...
      {SECS.map(sec=>{
        ...
        const isLabelSec=labelSecs.includes(sec.id);
        const naSec=notLabel&&isLabelSec; // 라벨 작업 아님 → 라벨 섹션 N/A
        return <Card key={sec.id} style={naSec?{opacity:.5}:{}}>
          <CardH style={{cursor:"pointer",userSelect:"none"}}
            ...
            {naSec
              ?<span style={{fontSize:10,fontWeight:700,color:C.orn}}>해당없음</span>
      ...
      <Card style={{padding:"16px 14px"}}>
        ...
        {/* 작업 가이드 보기 (해당 작업명에 등록된 링크 있을 때) */}
        {form.job&&guides[form.job.trim()]&&
          <a href={guides[form.job.trim()]} target="_blank" rel="noopener noreferrer"
            style={{display:"block",textAlign:"center",padding:"11px",marginBottom:10,
              background:"rgba(52,152,219,.12)",border:`1px solid ${C.blu}`,borderRadius:8,
              color:C.blu,fontSize:12,fontWeight:700,textDecoration:"none"}}>
            📄 '{form.job.trim()}' 작업 가이드 보기
          </a>}
        {/* 라벨 샘플 사진 첨부 (선택) */}
        <div style={{marginBottom:10,padding:"12px",background:C.sur,borderRadius:8,border:`1px solid ${C.bor}`}}>
          <div style={{fontSize:11,fontWeight:700,color:C.txt,marginBottom:6}}>📷 라벨 샘플 사진 <span style={{color:C.mut,fontWeight:400}}>(선택)</span></div>
          {!photoPreview
            ?<>
              ...
              <button type="button" onClick={()=>photoInputRef.current&&photoInputRef.current.click()}
                style={{display:"block",width:"100%",textAlign:"center",padding:"14px",
                  border:`1px dashed ${C.bor}`,background:"transparent",borderRadius:8,
                  color:C.blu,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                📷 사진 촬영 / 갤러리 선택
              </button>
            </>
            :<div style={{position:"relative"}}>
              <img src={photoPreview} alt="미리보기" style={{width:"100%",borderRadius:8,maxHeight:200,objectFit:"cover"}}/>
              <button onClick={()=>{setPhoto(null);setPhotoPreview("");}}
                style={{position:"absolute",top:6,right:6,width:28,height:28,borderRadius:"50%",
                  background:"rgba(0,0,0,.6)",color:"#fff",border:"none",fontSize:14,cursor:"pointer"}}>✕</button>
            </div>}
        </div>
        <Btn onClick={submitCL} bg={C.acc} disabled={done<total||uploading}>
          {uploading?"⏳ 사진 업로드 중...":"✅ 체크리스트 제출 및 작업 시작"}</Btn>
      </Card>

      {/* ── 내 제출 건 사진 추가 (제출 후 나중에 사진만 붙이기) ── */}
      {(()=>{
        const myName=(form.name||gs("lastName")||"").trim();
        const _today=(()=>{const n=new Date();return `${n.getFullYear()}.${n.getMonth()+1}.${n.getDate()}.`;})();
        // 내가 오늘 제출한 건들
        const myToday=myName?hist.filter(x=>x.w===myName&&x.dt===_today):[];
        if(myToday.length===0)return null;
        return <Card style={{marginTop:14}}>
          <CardH><span style={{fontSize:12,fontWeight:700,color:C.acc}}>📷 내 제출 건 사진 추가</span></CardH>
          <div style={{fontSize:10,color:C.mut,padding:"0 14px 8px",lineHeight:1.5}}>
            제출할 때 사진을 못 올렸다면 여기서 추가할 수 있습니다. (오늘 {myName}님 제출 건)
          </div>
          {/* 숨은 input - 사진 추가용 */}
          <input ref={addPhotoRef} type="file" accept="image/*" style={{display:"none"}}
            onChange={e=>{
              const f=e.target.files&&e.target.files[0];
              if(f&&addPhotoTs)addPhotoToRec(f,addPhotoTs);
              e.target.value="";
            }}/>
          {myToday.map((x,i)=>(
            <div key={x.ts||i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",
              borderBottom:`1px solid ${C.bor}`}}>
              {x.photo
                ?<img src={x.photo} alt="" style={{width:40,height:40,borderRadius:6,objectFit:"cover",flexShrink:0,border:`1px solid ${C.bor}`}}/>
                :<div style={{width:40,height:40,borderRadius:6,flexShrink:0,background:"#3a2d0f",
                  border:`1px solid ${C.orn}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>📷</div>}
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:12,color:C.txt}}><b>{x.it}</b>{x.job?` / ${x.job}`:""}</div>
                <div style={{fontSize:9,color:x.photo?C.grn:C.orn,marginTop:2,fontWeight:700}}>
                  {x.photo?"사진 첨부됨":"사진 없음"}
                </div>
              </div>
              {x.photo
                ?<span style={{fontSize:10,color:C.grn,flexShrink:0}}>✓</span>
                :<button onClick={()=>{setAddPhotoTs(x.ts);setTimeout(()=>addPhotoRef.current&&addPhotoRef.current.click(),50);}}
                  disabled={addingPhoto}
                  style={{padding:"7px 12px",background:C.orn,color:"#000",fontFamily:"inherit",
                    fontSize:11,fontWeight:700,border:"none",borderRadius:7,cursor:"pointer",flexShrink:0}}>
                  {addingPhoto&&addPhotoTs===x.ts?"업로드중":"사진 추가"}</button>}
            </div>
          ))}
        </Card>;
      })()}
    ...
    {tab==="edu"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      ...
      {/* ── 라인 역할별 R&R ── */}
      <Card style={{marginBottom:12,borderColor:C.acc,borderWidth:1}}>
        <CardH style={{justifyContent:"space-between",alignItems:"center"}}>
          <span onClick={()=>setRnrShow(v=>!v)} style={{fontSize:13,fontWeight:800,color:C.acc,cursor:"pointer",flex:1}}>👥 라인 역할별 R&R (책임·권한)</span>
          <span onClick={()=>{
            if(rnrEdit){setRnrEdit(false);}
            else{setRnrPinOpen(true);}
          }} style={{fontSize:10,fontWeight:700,color:rnrEdit?C.grn:C.mut,cursor:"pointer",marginRight:8,whiteSpace:"nowrap"}}>
            {rnrEdit?"✅ 완료":"✏️ 편집"}
          </span>
          <span onClick={()=>setRnrShow(v=>!v)} style={{fontSize:11,color:C.mut,transform:rnrShow?"none":"rotate(-90deg)",transition:"transform .2s",cursor:"pointer"}}>▼</span>
        </CardH>
        {rnrShow&&<div style={{padding:"0 12px 12px"}}>
          <div style={{fontSize:9.5,color:C.mut,marginBottom:10,lineHeight:1.5}}>
            {rnrEdit?"편집 모드: 각 역할의 부제·책임·핵심행동을 수정할 수 있습니다. 한 줄씩 고치고 완료를 누르세요.":"각 역할이 무엇을 책임지고 어디서 품질을 거르는지 정리했습니다. 인원이 바뀌어도 이 골격이 라인을 지탱합니다. 역할을 눌러 상세를 보세요."}
          </div>
          {rnrEdit&&<button onClick={()=>{if(confirm("R&R을 기본값으로 초기화할까요? 현재 편집 내용은 사라지고 최신 기본 구조(반장·조장·부조장·피킹·검수자·부착작업자)로 되돌립니다."))saveRnr(RNR);showToast("R&R이 기본값으로 초기화되었습니다",C.grn);}}
            style={{width:"100%",marginBottom:10,padding:"9px",background:"rgba(245,166,35,.12)",color:C.acc,
              border:`1px solid ${C.acc}`,borderRadius:8,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            ♻️ R&R 기본값으로 초기화 (옛 데이터 제거)</button>}
          {rnr.map((r,i)=>{
            const cmap={red:C.red,blu:C.blu,orn:C.orn,grn:C.grn,mut:C.mut,acc:C.acc};
            const col=cmap[r.color]||C.mut;
            const open=rnrEdit||rnrOpen===i;
            return <div key={i} style={{marginBottom:7,borderRadius:9,overflow:"hidden",
              border:`1px solid ${open?col:C.bor}`}}>
              <div onClick={()=>setRnrOpen(open?null:i)}
                style={{display:"flex",alignItems:"center",gap:9,padding:"10px 11px",cursor:"pointer",
                  background:open?`${col}22`:C.sur}}>
                <span style={{fontSize:17,flexShrink:0}}>{r.icon}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,fontWeight:800,color:open?col:C.txt}}>{r.role}</div>
                  <div style={{fontSize:9,color:C.mut,marginTop:1}}>{r.sub}</div>
                </div>
                <span style={{fontSize:10,color:C.mut,transform:open?"none":"rotate(-90deg)",transition:"transform .2s"}}>▼</span>
              </div>
              {open&&<div style={{padding:"11px",background:C.bg}}>
                {rnrEdit?(()=>{
                  const upd=(field,val)=>{saveRnr(rnr.map((x,j)=>j===i?{...x,[field]:val}:x));};
                  const updArr=(field,k,val)=>{const a=[...r[field]];a[k]=val;upd(field,a);};
                  const addLine=(field)=>{upd(field,[...r[field],"새 항목"]);};
                  const delLine=(field,k)=>{upd(field,r[field].filter((_,j)=>j!==k));};
                  return <div>
                    <div style={{fontSize:9,color:C.mut,marginBottom:3}}>부제</div>
                    <input value={r.sub} onChange={e=>upd("sub",e.target.value)}
                      style={{width:"100%",padding:"7px",marginBottom:9,background:C.card,color:C.txt,border:`1px solid ${C.bor}`,borderRadius:6,fontSize:11,boxSizing:"border-box"}}/>
                    <div style={{fontSize:9.5,fontWeight:700,color:col,marginBottom:5}}>■ 책임 (R&R)</div>
                    {r.resp.map((t,k)=>(
                      <div key={k} style={{display:"flex",gap:5,marginBottom:4,alignItems:"flex-start"}}>
                        <textarea value={t} onChange={e=>updArr("resp",k,e.target.value)}
                          style={{flex:1,padding:"6px",background:C.card,color:C.txt,border:`1px solid ${C.bor}`,borderRadius:6,fontSize:10,fontFamily:"inherit",minHeight:36,resize:"vertical",boxSizing:"border-box"}}/>
                        <span onClick={()=>delLine("resp",k)} style={{color:C.red,fontSize:14,cursor:"pointer",padding:"4px",flexShrink:0}}>×</span>
                      </div>
                    ))}
                    <div onClick={()=>addLine("resp")} style={{fontSize:10,color:C.blu,cursor:"pointer",marginBottom:9}}>+ 책임 추가</div>
                    <div style={{fontSize:9.5,fontWeight:700,color:col,margin:"5px 0 5px"}}>■ 핵심 행동</div>
                    {r.key.map((t,k)=>(
                      <div key={k} style={{display:"flex",gap:5,marginBottom:4,alignItems:"flex-start"}}>
                        <textarea value={t} onChange={e=>updArr("key",k,e.target.value)}
                          style={{flex:1,padding:"6px",background:C.card,color:C.txt,border:`1px solid ${C.bor}`,borderRadius:6,fontSize:10,fontFamily:"inherit",minHeight:36,resize:"vertical",boxSizing:"border-box"}}/>
                        <span onClick={()=>delLine("key",k)} style={{color:C.red,fontSize:14,cursor:"pointer",padding:"4px",flexShrink:0}}>×</span>
                      </div>
                    ))}
                    <div onClick={()=>addLine("key")} style={{fontSize:10,color:C.blu,cursor:"pointer",marginBottom:9}}>+ 핵심 행동 추가</div>
                    <div style={{fontSize:9,color:C.mut,marginBottom:3}}>역할 설명</div>
                    <textarea value={r.hand} onChange={e=>upd("hand",e.target.value)}
                      style={{width:"100%",padding:"7px",background:C.card,color:C.txt,border:`1px solid ${C.bor}`,borderRadius:6,fontSize:10,fontFamily:"inherit",minHeight:44,resize:"vertical",boxSizing:"border-box"}}/>
                    <button onClick={()=>{if(confirm(`'${r.role}' 역할을 삭제할까요?`))saveRnr(rnr.filter((_,j)=>j!==i));}}
                      style={{width:"100%",marginTop:8,padding:"8px",background:"#3d1a1a",color:C.red,
                        border:`1px solid ${C.red}`,borderRadius:7,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                      🗑 이 역할 삭제</button>
                  </div>;
                })():<div>
                <div style={{fontSize:9.5,fontWeight:700,color:col,marginBottom:5}}>■ 책임 (R&R)</div>
                {r.resp.map((t,k)=>(
                  <div key={k} style={{fontSize:10,color:C.txt,lineHeight:1.5,marginBottom:3,paddingLeft:8}}>· {t}</div>
                ))}
                <div style={{fontSize:9.5,fontWeight:700,color:col,margin:"9px 0 5px"}}>■ 핵심 행동</div>
                {r.key.map((t,k)=>(
                  <div key={k} style={{fontSize:10,color:C.txt,lineHeight:1.5,marginBottom:3,paddingLeft:8}}>· {t}</div>
                ))}
                <div style={{marginTop:9,padding:"8px 10px",background:`${col}18`,borderRadius:7,
                  borderLeft:`3px solid ${col}`}}>
                  <div style={{fontSize:9.5,color:C.txt,lineHeight:1.5}}>{r.hand}</div>
                </div>
                </div>}
              </div>}
            </div>;
          ...
          {/* 품질 게이트 흐름 요약 */}
          <div style={{marginTop:10,padding:"10px",background:C.sur,borderRadius:8,border:`1px solid ${C.bor}`}}>
            <div style={{fontSize:9.5,fontWeight:700,color:C.acc,marginBottom:6}}>🔗 품질이 걸러지는 단계</div>
            <div style={{fontSize:9.5,color:C.txt,lineHeight:1.7}}>
              <b style={{color:C.red}}>반장</b>(인원 배치·라벨 불출·1차 검수·매칭) → <b style={{color:C.blu}}>조장</b>(작업 지시·2차 매칭) → 부착작업자(1차 자기확인) → 부조장(작업 중 보조 확인) → <b style={{color:C.grn}}>검수자(작업 중 상시 확인)</b>
            </div>
            <div style={{fontSize:8.5,color:C.mut,marginTop:5,lineHeight:1.4}}>
              ※ 반장이 인원을 배치하고 조장이 작업을 지시한다. 라벨-제품 매칭은 반장·조장이 두 번 거르고, 작업 중에는 부조장·검수자가 상시 확인한다.
            </div>
          </div>
        </div>}
      </Card>
    ...
    {/* ── 건의 탭 (작업자) ── */}
    {tab==="sugg"&&<div style={{padding:12,maxWidth:640,margin:"0 auto"}}>
      <Card style={{padding:"14px",marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:800,color:C.acc,marginBottom:4}}>💬 관리자에게 건의하기</div>
        <div style={{fontSize:10,color:C.mut,marginBottom:12,lineHeight:1.5}}>
          현장에서 불편하거나 개선하면 좋을 점을 자유롭게 적어주세요. 관리자가 확인합니다. (작성 내용은 관리자만 볼 수 있습니다)
        </div>
        <div style={{marginBottom:9}}>
          <div style={{fontSize:9,color:C.mut,marginBottom:3}}>이름</div>
          <Inp value={suggF.w} onChange={e=>setSuggF({...suggF,w:e.target.value})} placeholder="이름"/>
        </div>
        <div style={{marginBottom:11}}>
          <div style={{fontSize:9,color:C.mut,marginBottom:3}}>건의 내용</div>
          <textarea value={suggF.text} onChange={e=>setSuggF({...suggF,text:e.target.value})}
            placeholder="예: 작업대 조명이 어두워 라벨 확인이 힘듭니다. / 특정 품목 라벨이 잘 안 붙습니다."
            style={{width:"100%",minHeight:90,padding:"10px",background:C.card,color:C.txt,
              border:`1px solid ${C.bor}`,borderRadius:8,fontFamily:"inherit",fontSize:12,
              resize:"vertical",boxSizing:"border-box",lineHeight:1.5}}/>
        </div>
        <Btn onClick={submitSugg} bg={C.acc}>💬 건의 보내기</Btn>
      </Card>

      {/* 내가 보낸 건의 */}
      {(()=>{
        const myName=(suggF.w||gs("lastName")||"").trim();
        const mine=myName?sugg.filter(x=>x.w===myName):[];
        if(mine.length===0)return <div style={{fontSize:11,color:C.mut,textAlign:"center",padding:"10px"}}>
          보낸 건의가 여기에 표시됩니다.</div>;
        return <Card>
          <CardH><span style={{fontSize:12,fontWeight:700,color:C.txt}}>내가 보낸 건의</span></CardH>
          {mine.map((x,i)=>(
            <div key={x.ts||i} style={{padding:"10px 14px",borderBottom:`1px solid ${C.bor}`}}>
              <div style={{fontSize:11,color:C.txt,lineHeight:1.5}}>{x.text}</div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:5}}>
                <span style={{fontSize:9,color:C.mut}}>{x.dt}</span>
                <span style={{fontSize:9,fontWeight:700,color:x.done?C.grn:(x.read?C.blu:C.mut)}}>
                  {x.done?"✓ 처리됨":(x.read?"읽음":"전달됨")}
                </span>
              </div>
            </div>
          ))}
      ...
      {!mgrIn
        ...
        :<>
          ...
          {(()=>{
            ...
            const todayJobs=hist.filter(x=>x.dt===today);
            const submitCnt=todayJobs.length;
            const openCnt=todayJobs.filter(x=>!x.we).length;
            const doneCnt=todayJobs.filter(x=>x.we).length;
            // 평균 달성률 (목표·완료 설정된 작업)
            const rated=todayJobs.filter(x=>x.wage&&x.price&&x.completed!=null&&x.completed!=="");
            let avgRate=null;
            if(rated.length){
              const sum=rated.reduce((a,x)=>{
                const tgt=targetUPDof(x);const lp=livePaceOf(x);
                return a+(tgt>0&&lp?Math.round(lp.projUPD/tgt*100):0);
              },0);
              avgRate=Math.round(sum/rated.length);
            }
            const todayRw=rwhist.filter(x=>x.dt===today);
            const rwPending=todayRw.filter(x=>x.st==="대기").length;
            const totalMm=todayJobs.filter(x=>x.mm).reduce((a,x)=>a+Math.round(x.mm/60*10)/10,0);
            // 사진 첨부률 (오늘 제출 중 사진 있는 비율)
            const withPhoto=todayJobs.filter(x=>x.photo).length;
            const photoRate=submitCnt?Math.round(withPhoto/submitCnt*100):null;
            const cards=[
              [submitCnt,"오늘 제출",C.txt],
              [doneCnt,"마감 완료",C.grn],
              [openCnt,"진행중",C.acc],
              [avgRate!=null?avgRate+"%":"—","평균 달성률",avgRate==null?C.mut:(avgRate>=100?C.grn:avgRate>=90?C.orn:C.red)],
              [photoRate!=null?photoRate+"%":"—","사진 첨부율",photoRate==null?C.mut:(photoRate>=100?C.grn:photoRate>=70?C.orn:C.red)],
              [todayRw.length,"재작업 신고",todayRw.length?C.orn:C.mut],
              [rwPending,"승인 대기",rwPending?C.red:C.mut],
              [Math.round(totalMm*10)/10,"총 공수(h)",C.blu],
              [doneCnt?Math.round(todayJobs.filter(x=>x.completed).reduce((a,x)=>a+(parseInt(x.completed)||0),0)):0,"총 완료수량",C.txt]
            ];
            return <Card style={{padding:"14px"}}>
              <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:10}}>오늘 운영 요약</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                {cards.map(([v,l,c],i)=>(
                  <div key={i} style={{background:C.sur,borderRadius:8,padding:"12px 8px",textAlign:"center"}}>
                    <div style={{fontSize:22,fontWeight:900,color:c,lineHeight:1}}>{v}</div>
                    <div style={{fontSize:9,color:C.mut,marginTop:4}}>{l}</div>
                  </div>
                ))}
              </div>
            </Card>;
          ...
          {/* 재작업 유형별 통계 */}
          {(()=>{
            const cnt={"교차부착":0,"미부착":0,"오부착":0};
            rwhist.forEach(x=>{if(cnt[x.rtype]!=null)cnt[x.rtype]++;});
            const total=cnt["교차부착"]+cnt["미부착"]+cnt["오부착"];
            if(total===0)return null;
            const rows=[["교차부착",cnt["교차부착"],"#ff6b6b"],["미부착",cnt["미부착"],C.orn],["오부착",cnt["오부착"],C.blu]];
            return <Card style={{padding:"14px"}}>
              <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:4}}>재작업 유형별 통계 (누적)</div>
              <div style={{fontSize:9,color:C.mut,marginBottom:10}}>총 {total}건 · 가장 잦은 유형을 교육에 반영하세요</div>
              {rows.map(([name,n,col])=>(
                <div key={name} style={{marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3}}>
                    <span style={{color:C.txt,fontWeight:700}}>{name}</span>
                    <span style={{color:col,fontWeight:700}}>{n}건 ({total?Math.round(n/total*100):0}%)</span>
                  </div>
                  <div style={{height:7,background:C.bor,borderRadius:99,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${total?n/total*100:0}%`,background:col,borderRadius:99,transition:"width .4s"}}/>
                  </div>
                </div>
              ))}
            </Card>;
          ...
          {/* 작업자 건의사항 관리 */}
          {(()=>{
            const unread=sugg.filter(x=>!x.read).length;
            return <Card style={{padding:"14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                <span style={{fontSize:11,fontWeight:700,color:C.acc}}>💬 작업자 건의사항</span>
                {unread>0&&<span style={{fontSize:9,fontWeight:700,color:"#fff",background:C.red,
                  borderRadius:99,padding:"1px 7px"}}>새 건의 {unread}</span>}
              </div>
              <div style={{fontSize:9,color:C.mut,marginBottom:10,lineHeight:1.5}}>작업자가 보낸 건의입니다. 읽음·처리 상태를 표시하고, 처리 끝난 건은 삭제할 수 있습니다.</div>
              {sugg.length===0
                ?<div style={{fontSize:11,color:C.mut,textAlign:"center",padding:"10px 0"}}>받은 건의가 없습니다.</div>
                :sugg.map((x,i)=>(
                  <div key={x.ts||i} style={{padding:"11px",marginBottom:8,borderRadius:9,
                    background:x.read?C.bg:C.sur,border:`1px solid ${x.done?C.grn:(x.read?C.bor:C.acc)}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:6}}>
                      <div style={{flex:1,minWidth:0}}>
                        <span style={{fontSize:11,fontWeight:700,color:C.txt}}>{x.w}</span>
                        <span style={{fontSize:9,color:C.mut,marginLeft:6}}>{x.dt}</span>
                      </div>
                      <span style={{fontSize:9,fontWeight:700,flexShrink:0,
                        color:x.done?C.grn:(x.read?C.blu:C.acc)}}>
                        {x.done?"✓ 처리됨":(x.read?"읽음":"● 새 건의")}
                      </span>
                    </div>
                    <div style={{fontSize:11,color:C.txt,lineHeight:1.55,marginBottom:9,whiteSpace:"pre-wrap"}}>{x.text}</div>
                    <div style={{display:"flex",gap:6}}>
                      {!x.read&&<SmallBtn bg={C.blu} onClick={()=>suggSetState(x.ts,{read:true})}>읽음 표시</SmallBtn>}
                      {!x.done
                        ?<SmallBtn bg={C.grn} color="#000" onClick={()=>suggSetState(x.ts,{read:true,done:true})}>처리 완료</SmallBtn>
                        :<SmallBtn bg={C.card} color={C.mut} style={{border:`1px solid ${C.bor}`}} onClick={()=>suggSetState(x.ts,{done:false})}>처리 취소</SmallBtn>}
                      <SmallBtn bg="#3d1a1a" color={C.red} onClick={()=>suggDelete(x.ts)}>삭제</SmallBtn>
                    </div>
                  </div>
                ))}
            </Card>;
          ...
          {/* 불량 패턴 분석 (시간대·작업자·품목별) */}
          {(()=>{
            if(rwhist.length===0)return null;
            // 시간대별 집계 (ts → 시)
            const byHour={};
            // 작업자별
            const byWorker={};
            ...
            const byItem={};
            rwhist.forEach(x=>{
              if(x.ts){const h=new Date(x.ts).getHours();byHour[h]=(byHour[h]||0)+1;}
              if(x.w){byWorker[x.w]=(byWorker[x.w]||0)+1;}
              if(x.it){byItem[x.it]=(byItem[x.it]||0)+1;}
            });
            // 시간대 구간 묶기 (오전/점심후/오후/마감전)
            const slots=[
              {label:"오전 (9~12시)",keys:[9,10,11],col:C.blu},
              {label:"점심 후 (12~14시)",keys:[12,13],col:C.grn},
              {label:"오후 (14~17시)",keys:[14,15,16],col:C.orn},
              {label:"마감 전 (17~19시)",keys:[17,18],col:C.red},
            ];
            const slotCnt=slots.map(s=>({...s,n:s.keys.reduce((a,k)=>a+(byHour[k]||0),0)}));
            const slotMax=Math.max(1,...slotCnt.map(s=>s.n));
            const slotTotal=slotCnt.reduce((a,s)=>a+s.n,0);
            // 작업자 top3
            const workerTop=Object.entries(byWorker).sort((a,b)=>b[1]-a[1]).slice(0,3);
            // 품목 top3
            const itemTop=Object.entries(byItem).sort((a,b)=>b[1]-a[1]).slice(0,3);
            // 인사이트 자동 도출
            const peakSlot=slotCnt.slice().sort((a,b)=>b.n-a.n)[0];
            const insights=[];
            if(peakSlot&&peakSlot.n>0&&slotTotal>0&&peakSlot.n/slotTotal>=0.4)
              insights.push(`불량의 ${Math.round(peakSlot.n/slotTotal*100)}%가 '${peakSlot.label}'에 집중됩니다. 이 시간대 집중력 저하·피로를 점검하세요.`);
            if(itemTop[0]&&itemTop[0][1]>=3)
              insights.push(`'${itemTop[0][0]}' 품목에서 불량이 가장 잦습니다(${itemTop[0][1]}건). 이 품목 작업 기준·가이드를 재점검하세요.`);
            if(workerTop[0]&&workerTop.length>1&&workerTop[0][1]>=workerTop[1][1]*2&&workerTop[0][1]>=3)
              insights.push(`특정 작업자에 불량이 몰립니다. 비난이 아니라 추가 교육·작업 재배치 관점에서 살펴보세요.`);

            return <Card style={{padding:"14px"}}>
              <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:4}}>📊 불량 패턴 분석</div>
              <div style={{fontSize:9,color:C.mut,marginBottom:12,lineHeight:1.5}}>쌓인 재작업 신고로 "언제·누구·어떤 품목"에 불량이 몰리는지 봅니다. 추측이 아니라 데이터로 개선 포인트를 찾으세요.</div>

              {/* 시간대별 */}
              <div style={{fontSize:10,fontWeight:700,color:C.txt,marginBottom:7}}>⏰ 시간대별 발생</div>
              {slotCnt.map(s=>(
                <div key={s.label} style={{marginBottom:7}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,marginBottom:3}}>
                    <span style={{color:C.mut}}>{s.label}</span>
                    <span style={{color:s.col,fontWeight:700}}>{s.n}건</span>
                  </div>
                  <div style={{height:6,background:C.bor,borderRadius:99,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${s.n/slotMax*100}%`,background:s.col,borderRadius:99,transition:"width .4s"}}/>
                  </div>
                </div>
              ))}

              {/* 품목별 top3 */}
              {itemTop.length>0&&<div style={{marginTop:14}}>
                <div style={{fontSize:10,fontWeight:700,color:C.txt,marginBottom:7}}>📦 불량 잦은 품목 TOP {itemTop.length}</div>
                {itemTop.map(([name,n],i)=>(
                  <div key={name} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<itemTop.length-1?`1px solid ${C.bor}`:"none"}}>
                    <span style={{fontSize:10,color:C.txt}}>{i+1}. {name}</span>
                    <span style={{fontSize:10,color:C.orn,fontWeight:700}}>{n}건</span>
                  </div>
                ))}
              </div>}

              {/* 작업자별 top3 */}
              {workerTop.length>0&&<div style={{marginTop:14}}>
                <div style={{fontSize:10,fontWeight:700,color:C.txt,marginBottom:7}}>👤 작업자별 발생 TOP {workerTop.length}</div>
                {workerTop.map(([name,n],i)=>(
                  <div key={name} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<workerTop.length-1?`1px solid ${C.bor}`:"none"}}>
                    <span style={{fontSize:10,color:C.txt}}>{i+1}. {name}</span>
                    <span style={{fontSize:10,color:C.blu,fontWeight:700}}>{n}건</span>
                  </div>
                ))}
                <div style={{fontSize:8,color:C.mut,marginTop:6,lineHeight:1.4}}>※ 작업자별 수치는 비난이 아니라 교육·배치 개선의 참고용입니다.</div>
              </div>}

              {/* 자동 인사이트 */}
              {insights.length>0&&<div style={{marginTop:14,padding:"11px",background:"rgba(245,166,35,.1)",border:`1px solid ${C.acc}`,borderRadius:8}}>
                <div style={{fontSize:10,fontWeight:700,color:C.acc,marginBottom:5}}>💡 데이터가 말해주는 것</div>
                {insights.map((t,i)=>(
                  <div key={i} style={{fontSize:9.5,color:C.txt,lineHeight:1.5,marginBottom:i<insights.length-1?5:0}}>• {t}</div>
                ))}
              </div>}
            </Card>;
          })()}
          ...
          <Card style={{padding:"14px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <div style={{fontSize:11,fontWeight:700,color:C.acc}}>작업 가이드 링크</div>
              <SmallBtn bg={C.blu} color="#fff" onClick={()=>{setGdName("");setGdUrl("");setGdModal(true);}}>+ 등록</SmallBtn>
            </div>
            <div style={{fontSize:9,color:C.mut,marginBottom:10,lineHeight:1.5}}>작업명별로 PPT·Excel 등 가이드 링크를 등록하면, 작업자가 체크 탭에서 해당 작업명 입력 시 "가이드 보기" 버튼으로 열람합니다. (구글 드라이브 공유 링크 권장)</div>
            {Object.keys(guides).length===0
              ?<div style={{fontSize:11,color:C.mut,padding:"8px 0",textAlign:"center"}}>등록된 가이드가 없습니다.</div>
              :Object.entries(guides).map(([name,url])=>(
                <div key={name} style={{background:C.sur,border:`1px solid ${C.bor}`,borderRadius:8,padding:"10px",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{flex:1,minWidth:0,marginRight:8}}>
                      <div style={{fontSize:12,fontWeight:700,color:C.txt}}>{name}</div>
                      <div style={{fontSize:9,color:C.blu,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{url}</div>
                    </div>
                    <div style={{display:"flex",gap:4,flexShrink:0}}>
                      <SmallBtn bg={C.blu} color="#fff" onClick={()=>{setGdName(name);setGdUrl(url);setGdModal(true);}}>수정</SmallBtn>
                      <SmallBtn bg="#3d1a1a" color={C.red} onClick={()=>{
                        const g={...guides};delete g[name];saveGuides(g);showToast("가이드 삭제됨",C.red);
                      }}>삭제</SmallBtn>
                    </div>
                  </div>
                </div>
          ...
          {/* 과거 기록 조회 (최근 60일 이전 데이터 검색) */}
          <Card style={{padding:"14px"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:4}}>🔍 과거 기록 조회</div>
            <div style={{fontSize:9,color:C.mut,marginBottom:10,lineHeight:1.5}}>화면에는 최근 {RECENT_DAYS}일치만 표시됩니다. 그 이전 기록(사진 포함)은 여기서 날짜·작업자·품목으로 검색하세요. 데이터는 안전하게 보관되어 있습니다.</div>
            <button onClick={()=>{setArchOpen(true);setArchResult(null);}}
              style={{width:"100%",padding:10,background:C.blu,color:"#fff",fontFamily:"inherit",
                fontSize:12,fontWeight:700,border:"none",borderRadius:8,cursor:"pointer"}}>
              과거 기록 검색 열기
            </button>
          </Card>

          {/* 제출된 라벨 샘플 사진 (작업별 분류 열람) */}
          <Card style={{padding:"14px"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.acc,marginBottom:8}}>📷 제출된 라벨 샘플 사진 <span style={{fontSize:9,color:C.mut,fontWeight:400}}>(최근 {RECENT_DAYS}일)</span></div>
            {photos.length===0
              ?<div style={{fontSize:11,color:C.mut,padding:"8px 0",textAlign:"center"}}>제출된 사진이 없습니다.</div>
              :(()=>{
                // 작업명별 분류
                const jobs=[...new Set(photos.map(p=>p.job||"미지정"))];
                const filtered=photoFilter?photos.filter(p=>(p.job||"미지정")===photoFilter):photos;
                return <>
                  {/* 작업명 필터 */}
                  <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                    <button onClick={()=>setPhotoFilter("")}
                      style={{padding:"5px 10px",borderRadius:99,fontSize:10,fontWeight:700,cursor:"pointer",
                        background:photoFilter===""?C.acc:C.sur,color:photoFilter===""?"#000":C.mut,
                        border:`1px solid ${photoFilter===""?C.acc:C.bor}`}}>전체 ({photos.length})</button>
                    {jobs.map(j=>{
                      const cnt=photos.filter(p=>(p.job||"미지정")===j).length;
                      return <button key={j} onClick={()=>setPhotoFilter(j)}
                        style={{padding:"5px 10px",borderRadius:99,fontSize:10,fontWeight:700,cursor:"pointer",
                          background:photoFilter===j?C.acc:C.sur,color:photoFilter===j?"#000":C.mut,
                          border:`1px solid ${photoFilter===j?C.acc:C.bor}`}}>{j} ({cnt})</button>;
                    })}
                  </div>
                  {/* 썸네일 그리드 */}
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                    {filtered.map((p,i)=>(
                      <div key={i} onClick={()=>setPhotoView(p.url)}
                        style={{cursor:"pointer",borderRadius:8,overflow:"hidden",background:C.sur,border:`1px solid ${C.bor}`}}>
                        <img src={p.url} alt={p.it} style={{width:"100%",height:80,objectFit:"cover",display:"block"}}/>
                        <div style={{padding:"4px 5px"}}>
                          <div style={{fontSize:9,color:C.txt,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.it}</div>
                          <div style={{fontSize:8,color:C.mut,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.w} · {p.dt}</div>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <Card>
            <CardH style={{justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",gap:5}}>
                <SmallBtn bg={noPhotoOnly?C.red:C.sur} color={noPhotoOnly?"#fff":C.mut}
                  style={{border:`1px solid ${noPhotoOnly?C.red:C.bor}`}}
                  onClick={()=>setNoPhotoOnly(v=>!v)}>📷 미첨부만</SmallBtn>
                <SmallBtn bg={C.grn} color="#000" onClick={()=>exportCSV("cl",hist,showToast,setCsvView)}>CSV</SmallBtn>
              </div>
            </CardH>
            {(()=>{
              const list=noPhotoOnly?hist.filter(x=>!x.photo):hist;
              if(!list.length)return <div style={{padding:"18px 14px",fontSize:11,color:C.mut,textAlign:"center"}}>{noPhotoOnly?"사진 미첨부 건이 없습니다.":"이력이 없습니다."}</div>;
              return list.map((x,i)=>(
              <div key={x.ts||i} style={{padding:"10px 14px",borderBottom:`1px solid ${C.bor}`,
                display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                {/* 사진 썸네일 또는 ❌ */}
                {x.photo
                  ?<img src={x.photo} alt="" onClick={()=>setPhotoView(x.photo)}
                    style={{width:42,height:42,borderRadius:6,objectFit:"cover",flexShrink:0,cursor:"pointer",border:`1px solid ${C.bor}`}}/>
                  :<div style={{width:42,height:42,borderRadius:6,flexShrink:0,background:"#3d1a1a",
                    border:`1px solid ${C.red}`,display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:16,color:C.red}}>✕</div>}
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:9,fontWeight:700,marginTop:3,color:x.photo?C.grn:C.red}}>
                    {x.photo?"📷 사진 첨부":"📷 사진 없음"}
                  </div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0}}>
                  ...
                  <SmallBtn bg="#3d1a1a" color={C.red} onClick={()=>{
                    const h=hist.filter(y=>y!==x);setHist(h);ss("lh",h);}}>삭제</SmallBtn>
                </div>
              </div>
            ));})()}
          </Card>
          ...
          {/* 저작권 표시 */}
          <div style={{marginTop:16,padding:"12px",textAlign:"center",lineHeight:1.6}}>
            <div style={{fontSize:10,color:C.mut}}>ⓒ 2026 이진수. All rights reserved.</div>
            <div style={{fontSize:9,color:C.mut,marginTop:3}}>본 프로그램은 제작자가 개인적으로 개발한 저작물입니다.<br/>무단 복제·배포·상업적 이용을 금합니다.</div>
          </div>
        </>}
    </div>}
    ...
    {/* ── 모달: 가이드 등록/수정 ── */}
    <Modal open={gdModal} onClose={()=>setGdModal(false)} title="작업 가이드 링크" titleColor={C.blu}>
      <div style={{marginBottom:10}}>
        <div style={{fontSize:10,color:C.mut,marginBottom:4,fontWeight:700}}>작업명</div>
        <Inp value={gdName} onChange={e=>setGdName(e.target.value)} placeholder="라벨부착, 세트구성 등 (체크 탭 작업명과 일치)"/>
      </div>
      <div style={{marginBottom:10}}>
        <div style={{fontSize:10,color:C.mut,marginBottom:4,fontWeight:700}}>가이드 링크 (URL)</div>
        <Inp value={gdUrl} onChange={e=>setGdUrl(e.target.value)} placeholder="https://docs.google.com/..."/>
      </div>
      <div style={{fontSize:9,color:C.mut,lineHeight:1.5,background:C.sur,borderRadius:7,padding:"8px 10px"}}>
        구글 드라이브/스프레드시트/슬라이드는 "공유 → 링크가 있는 모든 사용자 보기"로 설정 후 링크를 붙여넣으세요. 작업명이 작업자가 입력하는 작업명과 정확히 일치해야 버튼이 표시됩니다.
      </div>
      <MbtnsRow onCancel={()=>setGdModal(false)} onOk={()=>{
        if(!gdName.trim()){showToast("작업명을 입력하세요",C.red);return;}
        if(!gdUrl.trim()){showToast("링크를 입력하세요",C.red);return;}
        let url=gdUrl.trim();
        if(!/^https?:\/\//i.test(url))url="https://"+url;
        saveGuides({...guides,[gdName.trim()]:url});
        setGdModal(false);showToast("가이드 저장됨",C.grn);
    ...
    <PinModal open={rnrPinOpen} onClose={()=>setRnrPinOpen(false)}
      title="R&R 편집 PIN"
      onOk={v=>{
        if(checkPin(v)){setRnrEdit(true);setRnrShow(true);setRnrPinOpen(false);}
        else showToast("PIN이 올바르지 않습니다",C.red);
      }}/>
    ...
    {/* ── 과거 기록 조회 모달 ── */}
    {archOpen&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:900,
      display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 12px",overflowY:"auto"}}
      onClick={()=>setArchOpen(false)}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bg,borderRadius:14,maxWidth:560,width:"100%",
        border:`1px solid ${C.bor}`,padding:18,maxHeight:"85vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <span style={{fontSize:15,fontWeight:800,color:C.blu}}>🔍 과거 기록 조회</span>
          <button onClick={()=>setArchOpen(false)} style={{width:30,height:30,borderRadius:"50%",
            background:C.card,color:C.mut,border:`1px solid ${C.bor}`,fontSize:15,cursor:"pointer"}}>✕</button>
        </div>
        {/* 검색 조건 */}
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <div style={{flex:1}}>
            <div style={{fontSize:9,color:C.mut,marginBottom:3}}>시작일</div>
            <input type="date" value={archFrom} onChange={e=>setArchFrom(e.target.value)}
              style={{width:"100%",padding:8,background:C.card,color:C.txt,border:`1px solid ${C.bor}`,
                borderRadius:7,fontFamily:"inherit",fontSize:12}}/>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:9,color:C.mut,marginBottom:3}}>종료일</div>
        ...
        <div style={{marginBottom:10}}>
          <div style={{fontSize:9,color:C.mut,marginBottom:3}}>작업자 / 품목 / 작업명 (선택)</div>
          <input value={archKw} onChange={e=>setArchKw(e.target.value)} placeholder="예: 송, 크림, 부착"
            style={{width:"100%",padding:8,background:C.card,color:C.txt,border:`1px solid ${C.bor}`,
              borderRadius:7,fontFamily:"inherit",fontSize:12}}/>
        </div>
        <button onClick={runArchSearch} disabled={archLoading}
          style={{width:"100%",padding:10,background:archLoading?C.mut:C.blu,color:"#fff",fontFamily:"inherit",
            fontSize:12,fontWeight:700,border:"none",borderRadius:8,cursor:"pointer",marginBottom:14}}>
          {archLoading?"검색 중...":"검색"}
        </button>
        {/* 결과 */}
        {archResult!=null&&<div>
          <div style={{fontSize:11,fontWeight:700,color:C.txt,marginBottom:8}}>검색 결과 {archResult.length}건</div>
          {archResult.length===0
            ?<div style={{fontSize:11,color:C.mut,textAlign:"center",padding:"16px 0"}}>해당 기록이 없습니다.</div>
            :archResult.map((x,i)=>(
              <div key={x.ts||i} style={{display:"flex",gap:8,padding:"8px 0",borderBottom:`1px solid ${C.bor}`,alignItems:"center"}}>
                {x.photo
                  ?<img src={x.photo} alt="" onClick={()=>setPhotoView(x.photo)}
                    style={{width:38,height:38,borderRadius:6,objectFit:"cover",flexShrink:0,cursor:"pointer",border:`1px solid ${C.bor}`}}/>
                  :<div style={{width:38,height:38,borderRadius:6,flexShrink:0,background:C.card,
                    border:`1px solid ${C.bor}`,display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:14,color:C.mut}}>—</div>}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:12,color:C.txt}}><b>{x.w}</b> / {x.it}{x.job?` / ${x.job}`:""}</div>
                  <div style={{fontSize:9,color:C.mut,marginTop:2}}>{x.dt}{x.ws?` | ${x.ws}→${x.we||"미마감"}`:""}{x.photo?" | 📷":""}</div>
                </div>
    ...
    {/* ── 사진 확대 보기 ── */}
    {photoView&&<div onClick={()=>setPhotoView("")}
      style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:1000,
        display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <img src={photoView} alt="확대" style={{maxWidth:"100%",maxHeight:"85%",borderRadius:8}}/>
      <button onClick={()=>setPhotoView("")}
        style={{position:"absolute",top:20,right:20,width:40,height:40,borderRadius:"50%",
          background:"rgba(255,255,255,.2)",color:"#fff",border:"none",fontSize:20,cursor:"pointer"}}>✕</button>
      <a href={photoView} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
        style={{position:"absolute",bottom:24,padding:"10px 20px",background:C.blu,color:"#fff",
          borderRadius:8,fontSize:12,fontWeight:700,textDecoration:"none"}}>새 탭에서 원본 보기</a>
    </div>}
